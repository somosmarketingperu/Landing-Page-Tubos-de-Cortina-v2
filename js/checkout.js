...
function validateStep3() {
    ...
    if (acceptTerms && !acceptTerms.checked) {
        showFieldError('cod-accept-terms', 'Debes aceptar los términos y políticas');
        valid = false;
    }
    ...
    if (tipo === 'boleta') {
        ...
        if (!dni || !/^(\d{8}|\d{11})$/.test(dni)) { showFieldError('cod-dni', 'DNI (8 dígitos) o RUC (11 dígitos)'); valid = false; }
...
function handleConfirm() {
    ...
    const waUrl = 'https://wa.me/' + WA_EMPRESA + '?text=' + encodeURIComponent(lines);
    const shortText = encodeURIComponent('Hola. Mi Orden de Compra es: ' + ocN + '. Comprador: ' + nombre);
...
async function handleEmailResend() {
    ...
    if (ok) {
        ...
        setTimeout(function() { btn.textContent = '✉️ Re-enviar PDF'; }, 3000);
    }
}
